...
**Scope includes the entire Inbox feature, plus the Agents/Responders view next door.** Everything in `~/Developer/code/apps/code/src/renderer/features/inbox/` is in scope: the report list + detail, every adjacent affordance (banners, deep-link sync, engagement tracking, keyboard navigation, dismiss flows). **Additionally**, the desktop's Agents view at `~/Developer/code/apps/code/src/renderer/features/agents/components/AgentsView.tsx` (route `/code/agents`) — which mounts `ConfigureAgentsSection` from `features/inbox/` — is also in scope. On cloud, the equivalent surface is currently the `SourcesModal` (entered via the Inbox toolbar). Sync rule: the cloud Configure-agents surface must cover every section the desktop Agents view exposes — sources (always), per-user + team autonomy (always), connections (GitHub + Slack), setup-task CTA, and MCP servers — *or* explicitly justify a skipped section under Open questions with the product reason. The Inbox toolbar button on cloud is named **"Configure agents"** (matches desktop's Agents view label), NOT "Edit sources" / "Signal sources" / "Manage integrations".
...
- **The Configure-agents surface is in scope, every run.** Desktop has an Agents view at `/code/agents` whose body is `features/inbox/components/ConfigureAgentsSection.tsx`. It contains: (a) optional setup-task CTA gated on a flag, (b) Connections (GitHub integration + Slack notifications), (c) Auto-start threshold (team default + per-user override), (d) Responder source toggles, (e) MCP servers link-out. On cloud, the equivalent landing is whatever the Inbox toolbar's gear button opens — historically `SourcesModal`. Inspect `ConfigureAgentsSection` every run; if any section drifts in/out on desktop, the cloud modal follows. The toolbar button label is always **"Configure agents"**, not "Edit sources". Per-section porting decisions (cloud-modal vs sidebar scene vs drawer, which sections to omit) come from product questions during planning — but the inventory must enumerate every section so the question can be asked. Sections that get skipped go under "Skipped" in the final report with the product reason, never silently dropped.
...
- You called the Inbox toolbar's gear button "Edit sources" / "Signal sources" / "Manage integrations" → rename to **"Configure agents"** to match desktop's Agents view label.
- You forgot to inspect `~/Developer/code/apps/code/src/renderer/features/agents/components/AgentsView.tsx` → re-inspect; the Agents view body is in scope because it mounts `ConfigureAgentsSection` from `features/inbox/`. Treat its sections (Setup / Connections / Auto-start / Sources / MCP servers) as inbox parity items.
- You shipped the Configure-agents surface covering only "Sources" → check every section the desktop's `ConfigureAgentsSection` exposes today. Missing sections go under "Skipped" with the product reason, or get ported.
- You decided the Configure-agents surface shape (modal vs scene vs drawer) without asking → re-run planning; the surface is a product question, not a developer call.
