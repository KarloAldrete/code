...
import { McpInstallationsServiceEvent } from "./schemas";
import { McpInstallationsService } from "./service";
...
describe("McpInstallationsService.pollForOauthCompletion", () => {
  beforeEach(() => {
    vi.useFakeTimers();
  });
  afterEach(() => {
  ...
  it("emits Installed when pending_oauth flips to false", async () => {
    const responses = [
      okJson({
        results: [
    ...
    const service = new McpInstallationsService(auth as never);
    const handler = vi.fn();
    service.on(McpInstallationsServiceEvent.Installed, handler);
